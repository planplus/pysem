...
许多学者 (Heise, 1975; Duncan, 1975)曾描述过一个方法，通过检查结构模型的路径图得到变量见方差和协方差的表达式。在某些方面，Heise的方法很容易实现，因为是循序渐进的。但Duncan的方法使用直接和间接影响的区别更加有用。当您在考虑一个模型及其参数的识别时，试图制定包含变量之间的方差和协方差及其与模型参数的关系的方程时，这些规则通常会派上用场。当我们试图估计一个因变量对一个可能由几条路径或具有中间变量的路径连接的影响变量的总体影响时，它们就会派上用场。

假设因变量 $X_i$ 是变量 $Y_j$ 的直接影响，表示为 $X_i \xrightarrow[]{\gamma_{ji}} Y_j$，也就是说通过箭头直接连接 $X_i$ 和 $Y_j$。与箭头相关的结构化系数 $\gamma_{ji}$ 是 $X$ 对 $Y$ 的直接影响。结构化系数表示 $X_i$ 的单位变化对 $Y_j$ 变化的影响程度。现在，假设 $X_i$ 和 $Y_j$ 之间存在干预变量：

$$
X_i \xrightarrow[]{\gamma_{gi}} Y_g \xrightarrow[]{\alpha_{hg}} Y_h \xrightarrow[]{\alpha_{jh}} Y_j
$$

现在 $X_i$ 对 $Y_j$ 的影响是一种间接效应，因为这种效应必须由干预变量 $Y_g$ 和 $Y_h$来传导。$X_i$ 对 $Y_j$ 的间接影响值由 $X_i$ 到 $Y_j$ 的连接路径上的结构系数的乘积给出：$\gamma_{gi} \sdot \alpha_{hg} \sdot \alpha_{jh}$。这样可以简化表达式 $Y_j = \gamma_{ji}X_i = \gamma_{gi}\alpha_{hg}\alpha_{jh}X_i$。这里我们可以注意到，如果每个结构化系数的绝对大小都小于 $1.00$，那么 $X_i$ 对 $Y_j$ 的间接影响就小于任何干预变量在 $X_i$ 到 $Y_j$ 的开放路径之间的直接干预影响。说路径是开放的，是因为它不能返回到该路径中的第一个变量 $X_i$。DAG中的所有路径都是开放的。这就是说递归模型中的所有路径都是开放的。另一方面，如果路径反向与它的起始变量连接，则该路径称为环。对一个非递归模型来说，至少拥有一个环的图称为有向有环图。目前，我们只关注DAG和/或递归模型。

一个因变量可能对果变量有不止一个间接影响。从因变量到果变量，通过不同的干预变量可能有几种路径。干预变量的每一个不同的路径都对果变量有间接的影响。干预变量的路径是从果变量触发经过经过的干预变量的特定序列。如果序列中至少一个变量不出现在另一个序列中，则两个序列不同。因变量对果变量的 *总效应* 是其直接效用和所有间接效用的和。

我们首先来看两个变量之间的协方差。

## 矩阵方程

尽管普通的代数方程便于将模型输入计算机程序，但在理论研究上却不太方便。我们需要结构方程模型的矩阵方程表达方式，并从这些方程中推导出与因子分析类似的基本定理，即一个描述可观测变量方差-协方差矩阵的方程是模型参数矩阵的矩阵函数。

有两种方法可以用矩阵形式来表达具有潜在变量的结构方程模型的模型方程。第一种将扰动与外生变量分开处理：
...
或
$$
\eta^* = A\eta^*+\Gamma^*\xi^*+\Delta^*\epsilon^*
$$

其中 $\eta^*=\begin{bmatrix}\eta \\ y\end{bmatrix}$ 是内生变量 $(m+p)\times1$ 维分段随机向量。$\eta$ 是潜在内生变量的 $m \times 1$ 维向量，$y$ 是外显内生变量的 $p \times 1$ 维向量。$A$ 是内生变量之间的 $(m+p) \times (m+p)$ 维结构化系数矩阵。注意：$A$ 的主对角线通常为0，因为没有内生变量是自己的原因。（当 $A$ 的主对角线以上的所有元素都为0，所有非0元素都在主对角线以下时，我们就得到一个*递归模型*，即内生变量只受“上游”变量的影响，不受“下游”变量的影响。否则，当 $A$ 的主对角线以下和以上都有非0元素时，该模型为*非递归模型*。非递归模型包含反馈回路和诸如相互因果关系等结构，必须用不同于递归模型的方式来理解。）

$\Gamma^*$ 是将内生变量与外生变量联系起来的 $(m + p) \times (n + q)$ 维结构化参数矩阵。

$\xi^* = \begin{bmatrix}\xi \\ x\end{bmatrix}$ 是外生变量 $(n+q)\times1$ 维分段随机向量，其中 $\xi$ 是潜在外生变量的 $n \times 1$ 维向量，$x$ 是外显外生变量的 $q \times 1$ 维向量。

$\Psi^*$ 是联系外显内生变量和外生扰动变量的结构化参数的 $p \times p$ 维对角矩阵。

$\epsilon$ 是外显变量上扰动随机变量的 $p \times 1$ 维向量。

在某些处理中，$\Psi$ 被认为是一个单位矩阵，扰动的方差可以不均一。
